import { default as Encoding, Affine, Extend, Mix, Compose, ColorStop, ImageStub } from './Encoding.js';
    ...
    const recurse = node => {
      ...
      if ( node instanceof phet.scenery.Image ) {
        ...
        if ( canvas.width && canvas.height ) {
          ...
          str += `encoding.encode_image( new ImageStub( ${canvas.width}, ${canvas.height}, new Uint8Array( [ ${ [ ...imageData.data ].join( ', ' ) } ] ).buffer ) );\n`;
          this.encode_image( new ImageStub( canvas.width, canvas.height, buffer ) );
        }
      }
